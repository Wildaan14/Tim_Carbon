...
// ── NDVI RASTER SAMPLING ────────────────────────────────────
// Formula: IF(pixel_value >= 0.2181767280121414, (678.67 * pixel_value - 148.07) * 0.09, 0)
// Returns: { total: { areaHa, carbon, pixelCount }, byNama: { namobj: { kelas, areaHa, carbon } } }
async function calculateNDVIStockByFeatures(raster, features) {
  if (!features?.length) return { total: { areaHa: 0, carbon: 0, pixelCount: 0 }, byNama: {} };
  ...
  features.forEach((feat) => {
    ...
    const geom = feat.geometry;
    if (!geom?.coordinates) return;
    const rawRings = geom.type === "Polygon" ? [geom.coordinates[0]] : geom.type === "MultiPolygon" ? geom.coordinates.map((poly) => poly[0]) : [];
    rawRings.forEach((rawRing) => {
      let ring = rawRing;
      if (isUtm) {
        const { zone } = raster.crs;
  ...
  const total = { areaHa: 0, carbon: 0, pixelCount: 0 };
  const byNama = {};
  entries.forEach(({ key, namobj, kelas }) => {
    if (!byNama[key]) byNama[key] = { namobj, kelas, areaHa: 0, carbon: 0, pixelCount: 0 };
  });
  ...
  for (let j = 0; j < height; j++) {
    ...
    for (let i = 0; i < width; i++) {
      ...
      // NDVI Regression Formula
      if (val >= 0.2181767280121414) {
        const pxCarbon = (678.67 * val - 148.07) * 0.09;
        
        total.carbon += pxCarbon;
        total.areaHa += rowPxHa;
        total.pixelCount++;
        
        byNama[matchedKey].carbon += pxCarbon;
        byNama[matchedKey].areaHa += rowPxHa;
        byNama[matchedKey].pixelCount++;
      }
    }
  }
  return { total, byNama };
